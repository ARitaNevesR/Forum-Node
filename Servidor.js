...
//estas 3 variaveis são para salvar dados comuns para não estar sempre a repeti-los em todas as sessões!
    ...
    navbar2 += '<span><a href="/registo"><b>Registo</b></a></span>'
    navbar2 += '</nav>'
    navbar2 += '</div>'
    navbar2 += '</header>';
    navbar2 += '</body>\n';
...
var topo = '';
    ...
    topo += '<title>registo &amp; autenticação</title>\n';
    topo += '<style>'
...
servidor.get('/forum', function (req, res) {
    ...
    if (req.session.username) {
            ...
            //forum
            html += '</body>\n';
            html += '<h1 style="text-align:center;">Bem-vindo, ' + req.session.username + '</h1>\n'
            ...
            html += '<button type="submit" style="text-align: center; background: #FF91A4; border: 0; color: #fff; padding: 8px; font-family: \'Poppins\'; border-radius: 4px; cursor: pointer;">Procurar</button>';
            html += '</form>'
            html += '<a href="create" style="float: right;">'
            html += '<button style="background: #FF91A4; border: 0; color: #fff; padding: 8px; font-family: \'Poppins\'; border-radius: 4px; cursor: pointer;">Criar Tópico</button>'
            html += '</a>'
            html += '</div>'
            ...
            try {
                ...
                html += '<input type="radio" name="privacy" value="all" ' + (privacidadeSelectinada === 'all' ? 'checked' : '') + '>Todos ';
                html += '<input type="radio" name="privacy" value="public" ' + (privacidadeSelectinada === 'public' ? 'checked' : '') + '>Públicos ';
                html += '<input type="radio" name="privacy" value="private" ' + (privacidadeSelectinada === 'private' ? 'checked' : '') + '>Privados ';
                html += '<button type="submit" style="text-align: center;background: #FF91A4;border: 0;color: #fff;padding: 8px;font-family: \'Poppins\';border-radius: 4px;cursor: pointer;">Filtrar</button>';
                html += '</form>';
                html += '</div>'
                ...
                for (var i = 0; i < TopicsMostrar.length; i++) {
                    ...
                    html += '<h6 style="margin: 0;font-size: 0.9em;color: #555;">Criador: ' + TopicsMostrar[i].user + '</h6>\n';
                    ...
                    if (TopicsMostrar[i].privacy === ''){
                        ...
                        if (TopicsMostrar[i].privacy.includes(req.session.username)){
            ...
            html += navbar2;
            html += '<h1 style="text-align:center;">Bem-vindo</h1>\n';
            html += '<p style="text-align:center;">Gostava de te conhecer melhor! Regista-te ou inicia sessão no nosso forum!</p>'
        }
    html += fundo;
    res.type('html');
    res.send(html);
...
servidor.get('/chat', function (req, res) {
    ...
    html += '<input type="text" name="search" placeholder="Look for you friends!" style="padding: 10px;margin-top: 10px;margin-bottom: 20px;width: 24.5%;border: 1px solid #ddd;border-radius: 4px;box-sizing: border-box;">';
...
servidor.get('/privatechat/:username/:sessionUsername', function(req, res){
    ...
    if (req.session.username === sessionUsername && req.session.username !== username){
        if (registoUtilizadores && registoConversa !== null) {
            ...
            html += '<button type="submit" value="envia" style="text-align: center;background: #FF91A4;border: 0;color: #fff;padding: 10px;font-family: \'Poppins\';border-radius: 8px;cursor: pointer;">Enter</button>'
            html += '</form>\n';
            try {
...
servidor.get('/search', function (req, res) {
    ...
    html += topo;
    html += navbar1;
    html += '<h1 style="text-align:center;">Resultados para "' + search + '"</h1>\n';
    html += '<div style="text-align: center;">'
        html += '<form action="/search" method="GET">'
            ...
            html += '<button type="submit" style="text-align: center;background: #FF91A4;border: 0;color: #fff;padding: 10px;font-family: \'Poppins\';border-radius: 4px;cursor: pointer;">Procurar</button>'
        html += '</form>'
    html += '</div>'
    html += '<div style="text-align: right; margin-right: 420px;">'
        html += '<a href="create">'
            html += '<button style="text-align: center;background: #FF91A4;border: 0;color: #fff;padding: 10px;font-family: \'Poppins\';border-radius: 4px;cursor: pointer;">Criar Tópico</button>'
        html += '</a><br>\n';
    html += '</div>'
    try {
      var dadosFicheiro = fs.readFileSync("criado.json", "utf-8"); //conteúdo do ficheiro
    ...
    try {
        ...
        if (topic) {
            ...
            if (req.session.username === topic.user){
                html += '<form method="POST" action="/topic/' + id + '/delete">';
            ...
            html += '</div>'
            html += '<div style="max-width: 800px;margin: 40px auto;padding: 20px;background: #fff;border-radius: 20px; position:relative;">'
            html += '<h4> Edita o conteúdo aqui! </h4>'
            html += '<form method="POST" action="/topic/' + id + '/update">'
            ...
            html += '<div style="max-width: 800px;margin: 40px auto;padding: 20px;background: #fff;border-radius: 20px; position:relative;">';
            html += '<h2>Adiciona o teu comentário:</h2>';
            html += '<form method="POST" action="/topic/' + id + '/comment">';
            ...
            html += '<button type="submit" style="position: absolute; right: 22px; top: 60px; text-align: center; background: #FF91A4; border: 0; color: #fff; padding: 10px; font-family: \'Poppins\'; border-radius: 4px; cursor: pointer;">Comentar</button>';
            html += '</form>';
...
servidor.post('/processa_create', function (req, res) {
    ...
    html += topo;
    html += navbar1;
    html += '<h1 style="text-align:center;">Status Criar Tópico</h1>\n';
    if (title && content) {
    ...
    if (usernameExistente) {
        ...
        if (sucesso) {
            ...
            html += '<h3 style="text-align:center;">Falha ao criar tópico</h3>\n';
            html += '<div style="text-align:center;">'
            html += '<a href="create">'
...
servidor.get('/perfil', function (req, res) {
    ...
    if (req.session.username) { //o username aqui serve como uma variável de controlo
        ...
        html += topo;
        html += '<div style="max-width: 800px;margin: 40px auto;padding: 20px;background: #fff;border-radius: 20px; position:relative;">'
        html += '<h1 style="text-align:center;">Perfil</h1>'
        ...
        try {
            ...
            for (var i = 0; i < publicarFicheiro.length; i++) {
              var title = publicarFicheiro[i].title;
                  //aqui, basicamente se a privacidade do post estiver em branco ou tiver o teu user, estes vão aparecer na search! Caso contrário não vai!
                if (publicarFicheiro[i].user.includes(req.session.username)){
                    html += '<a href="/topic/' + publicarFicheiro[i].id + '" style="text-decoration:none;">';
        ...
        html += navbar2;
        html += topo;
        html += '<h1 style="text-align:center;">Não é possível aceder-se ao perfil</h1>'
        html += '<div style="text-align: center;">'
        html += '<a href="login">'
        html += '<button style="text-align: center;background: #FF91A4;border: 0;color: #fff;padding: 10px;font-family: \'Poppins\';border-radius: 4px;cursor: pointer;">Login</button>'
        html += '</a><br>\n';
